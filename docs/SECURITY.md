...
- Default development secrets (`JWT_SECRET_KEY`, `ENCRYPTION_MASTER_KEY`) are rejected at startup in staging/production; development emits a warning when defaults are used. Override them with strong, unique values before promoting a build.
